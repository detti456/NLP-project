...
def collect_pages(text):
    ...
    for line in text:
        ...
        if tag == 'page':
            ...
            id = m.group(3)
        elif tag == 'timestamp' and id: # <revision> <id></id> </revision>
            rev = m.group(3)
        elif tag == 'title':
            title = m.group(3)
        elif tag == 'redirect':
...
def process_dump(input_file, template_file, out_file, file_size,
                 process_count, html_safe=False, expand_templates=True):
    """
    ...
    if out_file == '-':
        ...
        output = OutputSplitter(nextFile, file_size, False)
    ...
    for id, rev, title, page in collect_pages(input):
        job = (id, rev, urlbase, title, page, ordinal)
...
def main():
    ...
    process_dump(input_file, None, output_path, file_size, default_process_count)
